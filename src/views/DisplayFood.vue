...
<AppHeader>
    ...
    <div class="font-bold text-lg">{{ restaurantName }}</div>
</AppHeader>
<div v-if="!foodStore.getFoodIsLoading" class="flex flex-col gap-4 m-auto p-2">
    <FoodCard @delete="handleDelete(restaurantId, food.foodId)" v-for="(food, index) in foodStore.foodOrdered" :key="`${index}-${food.name}`" :menu-item="food.name" :rating="food.rating" :comment="food.comment" :date="food.dateCreated" :file-name="food.fileName" />
</div>
</template>
